...
| `inference` | Model discovery, chat, and multi-modal inference (forecast, vision embed/similarity, text embedding, segmentation, detection, audio ASR, video embed) — modality-aware routing via `tenzro_forecast`, `tenzro_visionEmbed`, `tenzro_textEmbed`, `tenzro_segment`, `tenzro_detect`, `tenzro_transcribe`, `tenzro_videoEmbed`. Intent routing: `route_intent(&params)` (resolve an intent to a model without naming one), `chat_by_intent(&params, messages)` (resolve and run in one call), `orchestrate(&request)` (plan and run an ordered set of models, skills, tools, and agent/swarm delegation for a goal). Streaming: `chat_stream` (token stream), `chat_stream_channel` (per-token billing on a micropayment channel). `get_provenance(content_hash)` resolves the cached synthetic-content manifest (EU AI Act Art. 50(2)) for generated output. Content-addressed weights: `get_model_hash(id)` reads the canonical BLAKE3/SHA-256 record a fetcher verifies weights against before load, `list_model_hashes()` lists every recorded hash, `record_model_hash(id, &files)` anchors one (permissionless, first-recorder-wins) |
...
| `provider` | Hardware, model serving, scheduling. `serve_model(id)` auto-clusters when a model is too large for one host — the node reads the GGUF header for layer count, discovers LAN members from gossiped cluster announcements, and runs a layer-wise pipeline across them, no extra arguments needed. `serve_model_with(id, ServeOptions { force_cluster, force_single, visibility })` overrides placement (force a split or pin single-host) and `visibility` (`Network` gossips the model; `Private` keeps it local/LAN-only). Preview the layer split first with `discovery().cluster_preview(id, force, force_single)` (live node view; derives the shape from the GGUF header and discovers LAN members) |
| `discovery` | Network + distributed-serving discovery. LAN cluster preview (`cluster_preview`), local mDNS peers (`local_peers`). Distributed MoE: planning (`moe_shard_map`, `moe_plan_dispatch`, `moe_replication_policy`, `moe_catalog_shape`) and execution (`moe_expert_status` — resident experts with residency tier + footprint + whether GPU compute is active; `moe_prepare_experts` — slice a checkpoint into per-expert blobs, optionally block-quantized `q8_0` / `q4_k` / `q6_k` / `q4_k_m`; `moe_forward` — run one distributed layer forward, gating locally and fanning experts to holders over the `tenzro/moe` transport) |
| `hosting` | Decentralized app hosting via `client.hosting()` — publish a static site, a `wasi:http` function, or a Firecracker microVM and serve it under a public hostname (`myapp.apps.tenzro.xyz`). Sites: `publish_site` (builder), `get_site`, `list_sites`, `remove_site`, `set_alias`, `get_alias`, `list_aliases`, `remove_alias`, `set_placement`, `get_placement`, `list_placements`, `remove_placement`, `claim_domain`, `verify_domain`, `get_domain`, `list_domains`, `remove_domain`. Functions: `deploy_function` (builder), `get_function`, `list_functions`, `remove_function`. Machines: `deploy_machine` (builder), `get_machine`, `list_machines`, `remove_machine`, `machine_status`, `machine_sealing_key`. Leases: `list_leases`, `leases_for_app`. Content is uploaded as content-addressed blobs first (`client.iroh().publish_blob(...)`); a site is a route map of those hashes. Mutations are DID-owner-authenticated — set the signed envelope via `with_did_envelope(...)` first |
...
cargo run --example intent_orchestration
